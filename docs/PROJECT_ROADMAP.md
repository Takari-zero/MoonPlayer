...
- “我喜欢 / 最近播放 / 音乐文件夹”三个入口已作为紧凑快捷入口落地；无真实数据时应显示真实空状态。
...
- 点击歌曲右侧更多不误触播放。
- 点击迷你播放器信息区 / 月亮区域进入音乐播放页。
- 底部迷你播放器接入真实当前音频状态，支持上一曲 / 播放暂停 / 下一曲；三个控制按钮不会误进入播放页。
- 迷你播放器进度线位于控制器底部，支持点击 seek 和拖动 seek。
- 迷你播放器外层包裹区域透明，内部控制卡片保留；底部导航栏样式不随迷你播放器调整而变化。
- 新增 `RotatingMusicThumb`，迷你播放器左侧使用紫色月亮 / 月相方向旋转小方块，小方块整体慢速旋转，旋转裁切问题已处理。
- 音乐播放页第一版 UI 已完成：沉浸式展示，不显示底部导航栏；顶部只有下箭头和更多；下箭头关闭有下滑退出动画；右上三点菜单锚定在三点按钮下方；播放页使用月相黑胶盘方向；歌曲名、播放 / 暂停、上一曲 / 下一曲和进度 seek 接真实当前音频 / 队列。
- 播放歌曲时使用真实扫描音频列表作为队列，避免 Media3 队列只有当前单曲导致上一曲 / 下一曲失效。
- 顺序模式支持首尾循环；随机模式支持上一曲 / 下一曲，下一曲尽量不选当前歌曲，上一曲优先使用随机历史，随机源不使用固定 seed。
- 音乐主页“音乐文件夹”入口显示真实有音乐的文件夹；文件夹显示真实歌曲数量和大小；点击文件夹进入该文件夹歌曲列表；文件夹内歌曲点击只播放，不跳播放页。
- 文件夹 item 右侧三点菜单独立显示“从列表移除 / 删除音频文件”，并锚定在按钮下方。
- 页面右上角三点支持文件夹多选管理；文件夹多选支持取消、全选、从列表移除、删除；删除必须二次确认。
- 音乐主页歌曲多选栏位置移动到“全部歌曲”标题下方，不覆盖顶部栏；多选栏横向紧凑，删除文字不竖排。
- 歌曲多选和文件夹多选状态分离，避免互相误触。
- 新增通用 `MoonInlineSearchField`，音乐、视频、小说搜索体验对齐；视频文件夹详情页搜索也同步对齐。
- 底部导航栏图标已更新：小说 tab 使用打开书本图标，我的 tab 使用个人图标；底部导航栏整体样式、高度、选中态和功能逻辑不变。

仍未完成 / 后续：

- 完整歌词系统、歌词解析和同步、A-B 循环完整音频逻辑、定时停止完整逻辑、收藏持久化完整逻辑、复杂播放列表管理、专辑 / 歌手元数据系统、音频标签解析、音乐二级高级筛选、可视化频谱、后台通知控制增强。
- 音乐播放页底部功能入口中，歌词、A-B、定时、收藏等若仍为 Toast / 占位 / 后续能力，不应写成完整功能。
- 当前音乐文件夹删除只删除扫描到的音频文件，不删除非音频文件，不删除整个文件夹；权限不足时应提示，不绕过系统限制。
- 当前搜索统一只是统一顶部栏搜索框样式和行为，不代表重做视频 / 小说模块。
- 本轮不新增权限、依赖、Gradle 或 Manifest 变更，不修改视频播放页或小说阅读页。
